...
const UpdateRepurposeSchema = z.object({
    blogId: z.string().uuid(),
    format: z.string(),
    updatedContent: z.record(z.string(), z.any()),
});
...
export async function POST(req: NextRequest) {
    try {
        ...
        const result = UpdateRepurposeSchema.safeParse(body);

        if (!result.success) {
            return NextResponse.json({ error: 'Invalid request data', details: result.error.format() }, { status: 400 });
        }

        const { blogId, format, updatedContent } = result.data;
        ...
        // 2. Merge updated content
        const existingContent = (blog.repurposed_content as Record<string, any>) || {};
        const currentFormatData = existingContent[format] || {};
        ...
        console.error('Update Repurpose Error:', error);
