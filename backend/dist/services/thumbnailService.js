...
try {
    ...
    const fontsDir = path_1.default.join(__dirname, '../../public/fonts');
    (0, canvas_1.registerFont)(path_1.default.join(fontsDir, 'Inter-Bold.ttf'), { family: 'Inter', weight: 'bold' });
    (0, canvas_1.registerFont)(path_1.default.join(fontsDir, 'Inter-Regular.ttf'), { family: 'Inter', weight: 'normal' });
    fontsLoaded = true;
}
catch (e) {
...
async function generateThumbnail(data) {
    ...
    if (data.backgroundImageUrl) {
        try {
            const bgImg = await (0, canvas_1.loadImage)(data.backgroundImageUrl);
    ...
    if (data.userImageBuffer) {
        ...
        const img = await (0, canvas_1.loadImage)(await (0, sharp_1.default)(data.userImageBuffer)
        ...
        const personCanvas = (0, canvas_1.createCanvas)(personWidth, height);
    ...
    if (data.logoImageBuffer) {
        const logoImg = await (0, canvas_1.loadImage)(await (0, sharp_1.default)(data.logoImageBuffer).resize({ height: 45 }).png().toBuffer());
    ...
    return new Promise((resolve, reject) => {
        const uploadStream = cloudinary_1.v2.uploader.upload_stream({ folder: 'bloggen/thumbnails', format: 'webp' }, (error, result) => {
            if (error || !result)
                reject(error || new Error('Upload failed'));
            else
                resolve({ thumbnailUrl: result.secure_url, colors: data.colors });
        });
