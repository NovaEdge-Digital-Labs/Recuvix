...
BEGIN
    IF NOT EXISTS (
        ...
        AND (policyname = 'Users can view their own transactions' OR policyname = 'Users view own transactions')
    ) THEN
...
CREATE TABLE IF NOT EXISTS public.razorpay_orders (
    ...
    status TEXT NOT NULL DEFAULT 'created',-- Add credit system columns to profiles
ALTER TABLE public.profiles 
ADD COLUMN IF NOT EXISTS credits_balance INTEGER DEFAULT 0,
ADD COLUMN IF NOT EXISTS managed_mode_enabled BOOLEAN DEFAULT false;
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies 
        WHERE tablename = 'razorpay_orders' 
        AND (policyname = 'Users can view their own orders' OR policyname = 'Users view own orders')
    ) THEN
